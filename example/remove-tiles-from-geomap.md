...
[Geomap](https://d3plus.org/docs/#Geomap) tiles can be removed by setting the `tiles` property to false. Similarly, by setting `ocean` property to transparent will hide ocean on the map. To hide the zoom button at the top of the map, you can set `zoom` property to false. Following example cover all three of these properties:
